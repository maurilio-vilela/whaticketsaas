...
// [CORREÇÃO 1] Adicionado os novos parâmetros na tipagem da Query
type IndexQuery = {
  searchParam: string;
  pageNumber: string;
  status: string;
  ...
  tags: string;
  users: string;
  includeMessages: string; // Novo
  dateFrom: string; // Novo
  dateTo: string; // Novo
};
...
export const index = async (req: Request, res: Response): Promise<Response> => {
  const {
    ...
    withUnreadMessages,
    // [CORREÇÃO 2] Extraindo os parâmetros da requisição
    includeMessages,
    dateFrom,
    dateTo,
  ...
  const { tickets, count, hasMore } = await ListTicketsService({
    ...
    withUnreadMessages,
    companyId,
    // [CORREÇÃO 3] Passando os parâmetros para o Serviço
    includeMessages,
    dateFrom,
    dateTo,
  });
