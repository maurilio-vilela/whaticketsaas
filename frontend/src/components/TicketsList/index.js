...
const TicketsList = (props) => {
    ...
    // Determina o status efetivo: Se status (aba) for passado, usa ele.
    // Se não (aba busca), usa o statusFilter do modal.
    const effectiveStatus = status || statusFilter;

    // Reseta a lista sempre que os filtros mudam
    useEffect(() => {
        dispatch({ type: "RESET" });
        setPageNumber(1);
    }, [
    ...
    // Hook personalizado que busca na API
    const { tickets, hasMore, loading } = useTickets({
        ...
        // Novos parâmetros para o backend
        dateFrom,
        dateTo,
        includeMessages,
    });
    ...
    useEffect(() => {
        ...
        socket.on(`company-${companyId}-ticket`, (data) => {
            ...
            // A CURA: Se for um update, mas o backend não mandou o ticket, ignora.
        ...
        socket.on(`company-${companyId}-appMessage`, (data) => {
            // A CURA: Ignora confirmações de leitura, edições e reações que não afetam a lista lateral
    ...
    }, [effectiveStatus, showAll, user, selectedQueueIds]);
