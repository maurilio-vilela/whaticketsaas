...
const reducer = (state, action) => {
    ...
    if (action.type === "UPDATE_TICKET_PRESENCE") {
        ...
        if (ticketIndex !== -1) {
            const newState = [...state];
            newState[ticketIndex].presence = data.presence;
            return newState;
        }
        return state;
    }
...
const TicketsListCustom = (props) => {
    ...
    }, [tickets, effectiveStatus, searchParam, queues, profile]);

    useEffect(() => {
        const companyId = localStorage.getItem("companyId");
        const socket = socketManager.getSocket(companyId);

        // 1ª CAMADA DE BLINDAGEM: Lógica rigorosa de filtro de Socket
        const shouldUpdateTicket = (ticket) => {
            const belongsToUser = !ticket.userId || ticket.userId === user?.id || showAll;
            const belongsToSelectedQueue =
                selectedQueueIds.length === 0 || // Se não filtrou, aceita todos
                selectedQueueIds.includes(ticket.queueId) ||
                (ticket.queueId === null && profile !== "user"); // Admin pode ver ticket sem fila
        ...
        const handleReady = () => {
            if (effectiveStatus && effectiveStatus !== "all") {
                socket.emit("joinTickets", effectiveStatus);
            } else {
        ...
        const handleAppMessage = (data) => {
            ...
            if (data.action === "create" && shouldUpdateTicket(data.ticket)) {
                if (!effectiveStatus || effectiveStatus === "all" || data.ticket.status === effectiveStatus) {
    ...
    }, [effectiveStatus, status, showAll, user, selectedQueueIds, tags, users, profile, queues, socketManager]);
    ...
    let filteredTicketsList = ticketsList.filter((ticket) => ticket.isGroup.toString() === "false");
